...
func (aq *ArtistQuery) sqlAll(ctx context.Context) ([]*Artist, error) {
	...
	if query := aq.withAlbums; query != nil {
		...
		for _, n := range neighbors {
			fk := n.artist_albums
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "artist_albums" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "artist_albums" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Albums = append(node.Edges.Albums, n)
		}
	}
